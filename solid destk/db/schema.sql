...
-- 10. events · audit log persistente (cumplimiento + dashboards admin)
...
-- events (audit log): admin lee todos, user lee solo los suyos, insert solo el dueño
drop policy if exists "events_read"   on public.events;
drop policy if exists "events_insert" on public.events;
create policy "events_read"   on public.events for select to authenticated using (public.is_admin() or auth.uid() = user_id);
create policy "events_insert" on public.events for insert to authenticated with check (auth.uid() = user_id or user_id is null);
...
-- =====================================================================
-- Realtime publication · suscríbete a estos cambios desde el cliente
-- =====================================================================
